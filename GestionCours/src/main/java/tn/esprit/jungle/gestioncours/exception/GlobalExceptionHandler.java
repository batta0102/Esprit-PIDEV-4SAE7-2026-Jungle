...
public class GlobalExceptionHandler {
    ...
    /**
     * Aucun contrôleur ne correspond au chemin (souvent mauvais chemin ou {@code server.servlet.context-path}).
     */
    @ExceptionHandler(NoResourceFoundException.class)
    public ResponseEntity<ApiResponse<Object>> handleNoResourceFound(NoResourceFoundException ex, WebRequest request) {
        log.warn("No MVC mapping for {} {}", ex.getHttpMethod(), ex.getResourcePath());
        String hint = " Vérifiez server.servlet.context-path : avec context-path=/api, utilisez le mapping interne /recommend ou adaptez l’URL.";
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.error(
                "Aucun endpoint REST pour " + ex.getHttpMethod() + " /" + ex.getResourcePath() + "." + hint));
    }
