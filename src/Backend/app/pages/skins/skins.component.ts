...
export class SkinsComponent implements OnInit {
  ...
  readonly pageCount = computed(() => Math.max(1, Math.ceil(this.filteredSkins().length / this.pageSize)));

  readonly pagedSkins = computed(() => {
    const p = Math.min(this.page(), this.pageCount());
    const start = (p - 1) * this.pageSize;
    return this.filteredSkins().slice(start, start + this.pageSize);
  });
