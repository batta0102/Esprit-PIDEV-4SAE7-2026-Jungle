...
export class RecommendationService {
  ...
  getRecommendationsForProduct(productId: number, limit: number = 6): Observable<RecommendationProduct[]> {
    ...
    return this.http.get<RecommendationProduct[]>(url, this.httpOptions).pipe(
      tap(recommendations => {
        console.log(`[RecommendationService] Loaded ${recommendations.length} similar products`);
      }),
      map(recommendations => {
        if (!recommendations || recommendations.length === 0) {
          console.log('[RecommendationService] No similar products from backend, using random mock data');
          // Shuffle and return random mock products for demo
          return MOCK_RECOMMENDATIONS.sort(() => Math.random() - 0.5).slice(0, limit);
        }
        return recommendations;
      }),
      catchError(error => {
        console.error('[RecommendationService] Error loading similar products:', error);
        console.error('[RecommendationService] Using random mock data as fallback');
        return of(MOCK_RECOMMENDATIONS.sort(() => Math.random() - 0.5).slice(0, limit));
      })
    );
  }
}
